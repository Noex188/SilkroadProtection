    ...
    public class Packet
    {
        ...
        public void WriteAscii(String value, int code_page)
        {
            lock (m_lock)
            {
                ...
                string utf8_value = Encoding.UTF8.GetString(codepage_bytes);
                byte[] bytes = Encoding.Default.GetBytes(utf8_value);
        ...
        {
            WriteAsciiArray(values, 0, values.Length, 1251);
        }
        public void WriteAsciiArray(String[] values, int index, int count)
        {
            WriteAsciiArray(values, index, count, 1251);
        }
